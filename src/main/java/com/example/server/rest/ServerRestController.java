...
public class ServerRestController {
	...
	public ResponseEntity<ServerDTO> addClientOnServer (@RequestBody Server server) {
		...
		service.saveClient(server);
		System.out.println("данные ушли c Докер");
		System.out.println("serv док: "+server);
		return new ResponseEntity<>(ServerDTO.toServerDTO(server), HttpStatus.OK);
	}
